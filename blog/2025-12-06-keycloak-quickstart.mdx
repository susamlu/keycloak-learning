...
下载并解压 Keycloak 安装包，进入 Keycloak 根目录，启动开发模式的服务器（开发模式使用内置 H2 数据库，适合快速测试）：
...
# 以开发模式启动 Keycloak 服务器
bin/kc.sh start-dev
```

:::tip 成功启动标志
当终端打印出类似以下的日志信息时，说明 Keycloak 开发模式已成功启动：
...
:::warning 重要提示
开发模式仅用于测试和学习，生产环境必须使用 `start` 命令启动，并配置 HTTPS、外部数据库（如 PostgreSQL）、高可用集群等安全与高可用相关的配置参数。
:::

## 3. 配置 SSH 隧道【可选】

:::info 说明
本节仅适用于在远程服务器上部署 Keycloak，需要在本地访问的场景。如果 Keycloak 已安装在本地，可直接跳过本节，直接访问 [http://localhost:8080](http://localhost:8080)。
:::

如需在本地通过 localhost 访问远程服务器上的 Keycloak，需要建立 SSH 端口转发。本文以 macOS 系统为例，Windows 系统配置逻辑类似（核心为端口转发），但需适配系统调整具体命令（Windows 可使用 PowerShell 中的 SSH 命令或 PuTTY 工具）：
...
访问 [http://localhost:8080/admin](http://localhost:8080/admin)，输入刚才创建的管理员账号和密码，即可进入 Keycloak 管理页面，开始后续的 Realm、User、Client 配置。
...
3. 点击「Create」，创建 Realm，系统自动切换到 `myrealm` Realm。
...
在 `myrealm` Realm 中创建测试用户，用于后续登录验证（确保已切换到 `myrealm` Realm）：
...
Client 代表需要 Keycloak 保护的应用（如 Web 应用、移动应用等），需先在 Keycloak 中创建客户端才能接入认证流程：

1. 左侧菜单点击「Clients」→ 再点击「Create client」按钮；
2. 第一步「General settings」：
   - Client type：选择「OpenID Connect」（默认选项）
   - Client ID：输入 `myclient`（自定义标识符，需与应用配置中的客户端 ID 保持一致）
3. 点击「Next」，第二步「Capability config」保持默认（确保「Standard flow」已勾选，这是 OAuth 2.0 授权码流程，适用于浏览器登录）；
4. 点击「Next」，第三步「Login settings」：
   - Valid redirect URIs：输入应用的回调地址（测试可填 `https://www.keycloak.org/app/*`，正式环境需填写实际应用的回调地址，支持通配符 `*`）；
   - Web origins：输入 `https://www.keycloak.org`（测试用，正式环境填写应用访问地址，格式：`https://你的域名`，用于允许 CORS 跨域请求）；
5. 点击「Save」完成创建。
...
:::tip 常见问题
- **无法访问**：检查 Keycloak 服务是否已启动，确认端口 8080 未被占用
- **SSH 隧道断开**：保持 SSH 隧道终端开启，或使用 `-N -f` 参数后台运行
- **登录失败**：确认 Realm、Client、用户名和密码配置正确
:::
...
- **应用集成：** 通过 Keycloak 客户端库（如 Spring Boot、Node.js 等）将认证集成到自有应用中；
- **主题自定义：** 修改登录页面、管理控制台的样式，适配应用品牌风格；
- **生产环境优化：** 切换为 PostgreSQL 数据库、配置 HTTPS 证书、启用集群模式。
