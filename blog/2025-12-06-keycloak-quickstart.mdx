...
访问 [http://localhost:8080/admin](http://localhost:8080/admin)，输入刚才创建的管理员账号和密码，即可进入 Keycloak 管理界面，开始后续的 Realm、User、Client 配置。
...
3. 点击「Create」创建 Realm，系统会自动切换到 `myrealm` Realm。
...
4. 点击「Next」，第三步「Login settings」：
   ...
   - Web origins：输入 `https://www.keycloak.org`（测试用，正式环境填写实际应用的访问地址，格式：`https://你的域名`，用于允许 CORS 跨域请求）；
5. 点击「Save」完成创建。
...
- **应用集成：** 通过 Keycloak 官方适配工具（如 Spring Boot 适配器、Node.js SDK 等），将认证、授权能力集成到自有应用中；
